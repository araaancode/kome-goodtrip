...
const SingleDriverTicket = () => {
    let { driverId, stId } = useParams();
    ...
    const handleAddComment = () => {
        ...
        axios.put(`/api/admins/drivers/${driverId}/support-tickets/${stId}/add-comment`, { comment: newComment }, {
            headers: {
    ...
    useEffect(() => {
        ...
        axios.get(`/api/admins/drivers/${driverId}/support-tickets/${stId}`, { headers: { authorization: AuthStr } })
            .then(response => {
                setUser(response.data.data.assignedTo.name);
            ...
            <div className="container mx-auto p-4 rtl">
                <div className="mb-4">
                    {comments.map((comment) => (
                        <div
                            key={comment._id}
                            className={"user" in comment ? "bg-blue-100 shadow-md rounded-lg p-4 mb-4 text-right border" : "bg-yellow-100 shadow-md rounded-lg p-4 mb-4 text-right border"}
                        >

                            <div className='flex justify-between'>
                                {"user" in comment ? (<div className='flex justify-between'>
                                    <RiUser3Line />
...
export default SingleDriverTicket