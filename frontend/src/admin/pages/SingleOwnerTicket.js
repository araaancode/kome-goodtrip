...
const SingleOwnerTicket = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`/api/admins/owners/${ownerId}/support-tickets/${stId}`, { headers: { authorization: AuthStr } })
            .then(response => {
                setOwner(response.data.data.assignedTo.name);
            ...
            <div className="container mx-auto p-4 rtl">
                <div className="mb-4">
                    {comments.map((comment) => (
                        <div
                            key={comment._id}
                            className={"owner" in comment ? "bg-blue-100 shadow-md rounded-lg p-4 mb-4 text-right border" : "bg-yellow-100 shadow-md rounded-lg p-4 mb-4 text-right border"}
                        >

                            <div className='flex justify-between'>
                                {"owner" in comment ? (<div className='flex justify-between'>
                                    <RiUser3Line />
